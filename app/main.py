...
BRIEFING_PATH = os.path.join(_PROJECT_ROOT, "briefing_latest.md")
...
def write_dashboard_calendar(events: list[dict], tomorrow_events: list[dict] | None = None) -> None:
    ...
    If *tomorrow_events* is provided, they are appended with day='tomorrow'
    so the dashboard can render a separator.
    """
    try:
    ...
    def _format_events(raw_events, day_label="today"):
        result = []
    ...
    formatted = _format_events(events, "today")
    if tomorrow_events:
        formatted += _format_events(tomorrow_events, "tomorrow")
