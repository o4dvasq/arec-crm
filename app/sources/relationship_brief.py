...
def collect_relationship_data(org, offering, base_dir=None):
    ...
    upcoming_meetings = load_prospect_meetings(org, offering) or []
    ...
    for line in intel_content.split('\n'):
        ...
        if line.startswith('- **') or line.startswith('* **'):
            ...
            if m:
                ...
                # Normalize 'title' to 'role' — they represent the same concept
                if field == 'title':
                    field = 'role'
...
def collect_person_data(person_name, base_dir=None):
    ...
    # 2. Extract org name from intel files (Organization, Company, or Org fields)
    org_name = ''
    for pf in people_intel:
        org_match = re.search(
