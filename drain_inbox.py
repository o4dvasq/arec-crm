#!/usr/bin/env python3
"""
drain_inbox.py — AI Email Inbox drain script (CC-07 + CC-07a)

Reads unread messages from the ai@avilacapital.com shared mailbox (or a
named legacy folder with --folder), parses Oscar's intent note from
forwarded emails, and appends structured [AI Inbox] entries to inbox.md.
Cowork then triages inbox.md during /productivity:update.

Usage:
    python3 drain_inbox.py                     # drain shared mailbox
    python3 drain_inbox.py --folder=#Productivity  # legacy migration
"""
...
INBOX_PATH = os.path.expanduser("~/Dropbox/Tech/ClaudeProductivity/inbox.md")
...
def drain_inbox() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Drain AI email inbox into inbox.md"
    )
    parser.add_argument(
        "--folder",
        default=None,
        help="Legacy: drain from a specific mail folder (e.g. --folder='#Productivity')",
    )
    args = parser.parse_args()

    token = get_access_token()

    ai_inbox_email = os.environ.get("AI_INBOX_EMAIL", "ai@avilacapital.com")

    if args.folder:
        # Legacy migration: Oscar's personal mailbox, named folder
        print(f"[drain_inbox] Draining folder '{args.folder}' from personal mailbox")
        messages = get_folder_messages(token, folder=args.folder)
    else:
    ...
    for msg in messages:
        ...
        # Move to Processed folder (shared mailbox only; legacy folder drain skips this)
        if not args.folder:
            move_message(
                token,
