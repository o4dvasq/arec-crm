...
| ai@avilacapital.com shared mailbox (Oscar) | Same mailbox, now processes for all team members |
| Notion meeting transcripts (Oscar) | Meeting transcripts attributed to attending team members |
...
The existing ai@avilacapital.com shared mailbox pattern extends to the whole team.

**How it works:**
1. Team member forwards an email to ai@avilacapital.com
...
**Notion integration (existing pattern):**
- Meeting transcripts already flow into Notion
- Pipeline pulls transcripts for meetings with CRM contacts
- AI summarizes and extracts intelligence, attributed to attending team members

**Teams meeting transcripts (if available):**
- If meetings are recorded in Teams, transcripts are available via Graph
- Same processing: summarize, extract, attribute
...
[View full pipeline → https://crm.avilacapital.com/crm]
```
...
CREATE TYPE interaction_source AS ENUM (
    'manual', 'auto-graph', 'auto-notion', 'forwarded-email'
);
...
- Shared mailbox processor: ai@avilacapital.com for all team members
...
**Goal:** Meeting transcripts feed the intelligence pipeline.  
**Deliverable:** Notion and/or Teams transcripts processed and attributed.

- Notion API integration for transcript pull
- Teams transcript pull via Graph (if recordings enabled)
- AI processing: summarize, extract intelligence, detect signals
- Attribution to attending team members

---

### Phase I6: Cowork Bridge (Oscar Only)
**Goal:** Oscar's Cowork can read CRM intelligence and write via API.  
**Deliverable:** Cowork integration preserved.

- Markdown export from Postgres to Dropbox (scheduled)
- Service token for Cowork API writes
- Updated Cowork skill definitions
- Oscar's local morning briefing reads from Azure API (with fallback)
...
**9:30 AM** — After a meeting with CalPERS, Oscar opens crm.avilacapital.com, navigates to CalPERS, types: "Jane pushing for 2nd close commitment. Wants updated track record by Friday." The AI tags it `commitment_signal` + `document_request`.
...
8. ✅ Forwarded emails to ai@avilacapital.com processed and attributed
...
18. ✅ Transcripts automatically processed and attributed
19. ✅ Extracted intelligence appears on prospect timelines

### Phase I6 (Cowork Bridge)
20. ✅ Markdown export reflects database state
21. ✅ Cowork writes to CRM via API
22. ✅ Oscar's local briefing reads from Azure API
