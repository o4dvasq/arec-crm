...
> **⚠️ MIGRATION NOTE (March 12, 2026):** This spec was written before the Azure migration. The app now runs on PostgreSQL only — `crm_reader.py` is deleted. All references to `crm_reader.py` below should be read as `crm_db.py`. Implement `merge_organizations()` in `crm_db.py` (a stub already exists there). Do NOT create or import `crm_reader.py`. Work on `azure-migration` branch.
